    ...
    <div className="glass-panel rounded-2xl p-4 sm:p-5 border border-slate-800/80 shadow-2xl relative overflow-hidden">
      <div className="flex items-center justify-between mb-3 px-1">
        ...
        {selectedVote !== null && selectedVote !== undefined && !revealed && (
          <span className="text-xs text-brand-400 font-medium animate-fade-in">
            Selected: <strong className="text-white text-sm bg-brand-600/30 px-2 py-0.5 rounded border border-brand-500/40">{selectedVote}</strong>
          </span>
        )}
      </div>
          ...
          return (
            <button
              ...
              className={`group relative flex flex-col items-center justify-center h-16 sm:h-20 rounded-xl font-bold text-base sm:text-lg transition-all duration-150 transform active:scale-95 ${
                isSelected
                  ? 'bg-gradient-to-b from-brand-500 to-rose-700 text-white border-2 border-brand-300 shadow-lg shadow-brand-500/40 -translate-y-1.5'
                  : isDisabled
